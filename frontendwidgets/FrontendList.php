...
class FrontendList extends FrontendWidgetBase
{
    ...
    public function prepareModel()
    {
        ...
        if (method_exists($this->component, 'extendQueryBefore')) {
            $query = $this->component->extendQueryBefore($query);
        }
    ...
    public function injectRecordUrl($records)
    {
        if ($records->count() === 0) {
            return $records;
        }
        ...
        if (isset($this->recordOnClick)) {
            foreach ($records as &$record) {
                $record['url'] = 'javascript:;';
            }
        }

        $recordPage = $this->component->property('recordPage');

        $firstRecord = $records->first();//->toArray();
        $data = $firstRecord->toArray();

        $columns = array_keys($data);
        $columns[] = $firstRecord->getKeyName();
        ...
        if ($recordPage) {
            foreach ($records as &$record) {
                ...
                foreach ($columns as $column) {
                    $recordParams[$column] = $record[$column];
                }
                $record['url'] = Page::Url($recordPage, $recordParams);
            }
        }
    ...
    protected function defineListColumns()
    {
        if (!isset($this->columns) || !is_array($this->columns) || !count($this->columns)) {
            $class = get_class($this->model instanceof Model ? $this->model : $this->component);
    ...
    public function getColumnValue($record, $column)
    {
         ...
         *
         */
        if ($response = $this->fireSystemEvent('pc.frontend.list.overrideColumnValue', [$record, $column, &$value])) {
            $value = $response;
        }
     ...
     */
    public function getSetupPerPageOptions()
    {
     ...
     */
    public function getSetupListColumns()
    {
        /*
